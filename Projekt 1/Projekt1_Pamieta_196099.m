...
figure(3); imshow(FImP);
title({'Rzut perspektywiczny obu trójkątów (algorytm Z-bufora)',...
    'pozycja początkowa, brak przeźroczystości'});
...
figure(4); imshow(FImP);
title({'Rzut perspektywiczny obu trójkątów (algorytm Z-bufora)',...
    'pozycja początkowa, przeźroczystość trójkątów 75%, przeźroczystość drugiego planu: 25%'});

% transformacja pierwszego trójkąta - pozycja końcowa
H1 = RPYT(index,index,0,1,1,1);
r11 = H1*p11; r12 = H1*p12; r13 = H1*p13;

% transformacja drugiego trójkąta - pozycja końcowa
H2 = RPYT(0,index,index,1,1,-1);
r21 = H2*p21; r22 = H2*p22; r23 = H2*p23;
...
SH1P = inside(rp11,rp12,rp13,m,n,pix,pix,colort1); figure(5); imshow(SH1P);
title('Rzut perspektywiczny pierwszego trójkąta - pozycja końcowa');
SH2P = inside(rp21,rp22,rp23,m,n,pix,pix,colort2); figure(6); imshow(SH2P);
...
figure(7); imshow(FImP);
title({'Rzut perspektywiczny obu trójkątów (algorytm Z-bufora)',...
    'pozycja końcowa, brak przeźroczystości'});
...
figure(8); imshow(FImP);
title({'Rzut perspektywiczny obu trójkątów (algorytm Z-bufora)',...
    'pozycja końcowa, przeźroczystość trójkątów 75%, przeźroczystość drugiego planu: 25%'});

%% CZĘŚC 2

% video
v = VideoWriter('Projekt1_Pamięta_196099.avi');
open(v);
